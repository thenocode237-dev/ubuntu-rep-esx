...
function OpenShopMenu()
	...
	-- UBUNTU-RP : bouton de sortie visible (sinon on reste coincé sans acheter).
	table.insert(elements, { name = '__ubuntu_exit', label = 'Quitter la concession', value = 0 })
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		-- UBUNTU-RP : sortie propre (dégèle / rend visible / téléporte à l'entrée).
		if data.current.name == '__ubuntu_exit' then
			IsInShopMenu = false
			...
			SetEntityCoords(ped, Config.Zones.ShopEntering.Pos)
			menu.close()
			return
		end
		...
		if data.current.name == '__ubuntu_exit' then return end -- UBUNTU-RP : pas d'aperçu pour le bouton Quitter
		local vehicleData = vehiclesByCategory[data.current.name][data.current.value + 1]
		local playerPed   = PlayerPedId()
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'resell_vehicle' then
					ESX.TriggerServerCallback('esx_vehicleshop:resellVehicle', function(vehicleSold)
						if vehicleSold then
							ESX.Game.DeleteVehicle(CurrentActionData.vehicle)
							ESX.ShowNotification(TranslateCap('vehicle_sold_for', CurrentActionData.label,
